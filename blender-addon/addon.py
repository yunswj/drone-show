        ...
        for drone_obj in drone_objects:
            with open(os.path.join(self.filepath, '{}.csv'.format(drone_obj.name.lower())), 'w') as csv_file:
                ...
                for frame_number in range(frame_start, frame_end + 1):
                    ...
                    speed = calc_speed(
                        (x, y, z), (prev_x, prev_y, prev_z)) if frame_number != frame_start else 1
                ...
                if distance_exeeded:
                    self.report(
                        {'WARNING'}, "Drone '%s' distance limits exeeded" % drone_obj.name)
                self.report(
                    {'WARNING'}, "Animation file exported for drone '%s'" % drone_obj.name)
        return {'FINISHED'}
                    ...
                    for node in slot.material.node_tree.nodes:
                        if node.type in ('EMISSION', 'BSDF_DIFFUSE', 'BSDF_PRINCIPLED'):
                            ...
                            for component in range(3):
                                rgb[component] = int(
                                    node.inputs[0].default_value[component] * alpha * 255)
                else:
                    for component in range(3):
                        rgb[component] = int(
                            slot.material.diffuse_color[component] * 255)
                break

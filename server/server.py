...
class ExitMsgbox(logging.Handler):
    ...
    def _emit(self, record):
        # window.close()
        QMessageBox.warning(None, "Critical error in {}: {}". format(
            record.name, record.threadName), record.msg)
        QApplication.quit()
        sys.exit(record.msg)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def init_table(self):
        ...
        self.ui.copter_table = CopterTableWidget(
            self.model, self.server.config)
    ...
    def init_model(self):
        ...
        self.model.selected_ready_signal.connect(
            self.ui.start_button.setEnabled)
        self.model.selected_takeoff_ready_signal.connect(
            self.ui.takeoff_button.setEnabled)
        self.model.selected_flip_ready_signal.connect(
            self.ui.flip_button.setEnabled)
        # Connect calibrating signal (testing)
        self.model.selected_calibrating_signal.connect(
            self.ui.check_button.setDisabled)
        self.model.selected_calibrating_signal.connect(
            self.ui.pause_button.setDisabled)
        self.model.selected_calibrating_signal.connect(
            self.ui.land_all_button.setDisabled)
        self.model.selected_calibrating_signal.connect(
            self.ui.land_selected_button.setDisabled)
        self.model.selected_calibrating_signal.connect(
            self.ui.disarm_selected_button.setDisabled)
        self.model.selected_calibrating_signal.connect(
            self.ui.disarm_all_button.setDisabled)
        self.model.selected_calibrating_signal.connect(
            self.ui.visual_land_button.setDisabled)
        self.model.selected_calibrating_signal.connect(
            self.ui.emergency_land_button.setDisabled)
        self.model.selected_calibrating_signal.connect(
            self.ui.leds_button.setDisabled)
        self.model.selected_calibrating_signal.connect(
            self.ui.reboot_fcu.setDisabled)

        self.model.selected_calibration_ready_signal.connect(
            self.ui.calibrate_gyro.setEnabled)
        self.model.selected_calibration_ready_signal.connect(
            self.ui.calibrate_level.setEnabled)
    ...
    # TODO merge with client_connection_changed
    ...
    def client_connection_changed(self, client: Client):
        logging.debug("Connection {} changed {}".format(
        ...
        if self.server.config.table_remove_disconnected and (not client.connected):
            ...
            if row_num is not None:
                self.model.update_data(
                    row_num, 0, client.connected, table.ModelStateRole)
    ...
    def send_start_time_selected(self):
        ...
        if self.ui.music_checkbox.isChecked():
            ...
            asyncio.ensure_future(self.play_music_at_time(
                music_dt + time_now + time_lag), loop=loop)
    ...
    def pause_resume_selected(self):
        if self.ui.pause_button.text() == 'Pause':
            ...
            self.send_to_selected("resume", command_kwargs={
                                  "time": server.time_now() + time_gap})
        ...
        for copter in self.model.user_selected():
            if table.takeoff_checks(copter):
                if self.ui.z_checkbox.isChecked():
                    copter.client.send_message("takeoff_z", kwargs={"z": str(
                        self.ui.z_spin.value())})  # todo int, merge commands
                else:
    ...
    def _send_files(self, files, copters=None, client_path="", client_filename="", match_id=False, callback=None, clover_dir=False):
        ...
        for num, file in enumerate(files):
            ...
            logging.info("Preparing file for sending: {} {}".format(
                filepath, filename))

            if match_id:
                name = os.path.splitext(filename)[0]
                to_send = [copter for copter in copters if re.fullmatch(
                    name, copter.copter_id)]
            else:
                to_send = copters
            ...
            for copter in to_send:
                if clover_dir:
                    if copter.client.clover_dir != 'error':
                        path_to_send = os.path.realpath(os.path.join(
                            copter.client.clover_dir, client_path))
                    else:
                        logging.error(
                            "Can't send files to clover ROS package on {}".format(copter.copter_id))
                else:
                    path_to_send = client_path
                copter.client.send_file(
                    file, os.path.join(path_to_send, filename))
    ...
    def send_files(self, prompt, ext_filter, copters=None, client_path="", client_filename="", match_id=False,
        ...
        if onefile:
            file = QFileDialog.getOpenFileName(
                self, prompt, filter=ext_filter)[0]
        ...
        self._send_files(files, copters, client_path,
                         client_filename, match_id, callback, clover_dir)
    ...
    def request_any_file(self, client_path=None, copters=None):
        ...
        logging.info(
            f'Requesting file {client_path} to  local {save_path} from clients: {copters}')
        for copter in copters:
            if len(copters) > 1:
                save_path = cfg.modify_filename(
                    save_path, f"{{}}_{copter.copter_id}")
    ...
    def send_any_file(self):
        ...
        files = [file]
        self._send_files(files, client_path=c_filepath,
                         client_filename=c_filename)
    ...
    def send_animation(self):
        self.send_files("Select animation file", "Animation files (*.csv)",
                        onefile=True, client_filename="animation.csv")
    ...
    def send_aruco(self):
        def callback(copter):
            copter.client.send_message(
                "service_restart", kwargs={"name": "clover"})
    ...
    def send_fcu_parameters(self):
        def request_callback(client, value):
            logging.info("Send parameters to {} success: {}".format(
                client.copter_id, value))
    ...
    def send_config(self):
        ...
        path = QFileDialog.getOpenFileName(
            self, "Select configuration file", filter="Configs (*.ini *.txt *.cfg)")[0]
        if not path:
            return
        ...
        for copter in copters:
            copter.client.send_message(
                "config", kwargs={"config": data, "mode": mode.lower()})
    ...
    def restart_clever_show(self):
        for copter in self.model.user_selected():
            copter.client.send_message("service_restart", kwargs={
                                       "name": "clever-show"})
    ...
    def select_music_file(self):
        path = QFileDialog.getOpenFileName(
            self, "Select music file", filter="Music files (*.mp3 *.wav)")[0]
        if not path:
            return
        ...
        self.ui.action_select_music_file.setText(
            self.ui.action_select_music_file.text() + " (selected)")
    ...
    def edit_server_config(self):
        ...
        ConfigDialog().call_config_dialog(
            config, save_callback, restart, name="server config")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(
                current_dir, "server_logs", "{}.log".format(now))),
    ...
    progressBar.setGeometry(25, splash_pix.height() -
                            80, splash_pix.width(), 35)
    ...
    app_icon = QIcon()
    app_icon.addFile(os.path.join(current_dir, "icons",
                     "image.ico"), QtCore.QSize(256, 256))
    ...
    # app.exec_()
    with loop:
        server = ServerQt(config_path=os.path.join(
            current_dir, "config", "server.ini"))

...
formatter = logging.Formatter(
    "%(asctime)s [%(name)-7.7s] [%(threadName)-12.12s] [%(levelname)-5.5s]  %(message)s")
...
class CopterClient(client_core.Client):
    ...
    def start(self, task_manager_instance):
        ...
        client_thread = threading.Thread(target=super(
            CopterClient, self).start, name="Client thread")
    ...
    def start_floor_frame_broadcast(self):
        ...
        trans.transform.rotation = Quaternion(*quaternion_from_euler(math.radians(self.config.floor_frame_rotation[0]),
                                                                     math.radians(
                                                                         self.config.floor_frame_rotation[1]),
    ...
    def start_gps_frame_broadcast(self):
        gps_frame_thread = threading.Thread(
            target=self.gps_frame_broadcast_loop, name="GPS frame broadcast thread")
            ...
            if telem is not None:
                if contains_nan([telem.lat, telem.lon, telem.x, telem.y, telem.yaw, amsl_height, compass_hdg]):
                    ...
                    if self.config.gps_frame_amsl != "current":
                        try:
                            gps_dz = telem.z - amsl_height + \
                                float(self.config.gps_frame_amsl)
                        except ValueError:
                            logger.error(
                                "Wrong amsl height in GPS FRAME config!")
                            continue
                    ...
                    yaw = math.radians(compass_hdg) + telem.yaw - \
...
def configure_bashrc(hostname):
    ...
    if current_hostname != hostname:
        content = raw_content[:index_start] + \
            hostname + raw_content[index_stop:]
        try:
            with open(path, 'w') as f:
                f.write(content)
        except IOError:
...
def _command_move_start_to_current_position(*args, **kwargs):
    ...
    offset = numpy.array(private_offset) + \
        numpy.array(copter.config.animation_common_offset)
    try:
        xs, ys, zs = copter.animation.get_start_frame(
            copter.telemetry.start_action).get_pos()
    except ValueError:
...
def _command_reset_start(*args, **kwargs):
    copter.config.set('ANIMATION', 'private_offset', [
                      0, 0, copter.config.animation_private_offset[2]], write=True)
    logger.info("Reset start to {:.2f} {:.2f}".format(
        copter.config.animation_private_offset[0], copter.config.animation_private_offset[1]))
...
def _command_set_z(*args, **kwargs):
    ...
    if valid([telem.x, telem.y, telem.z]):
        ...
        logger.info("Set z offset to {:.2f}".format(
            copter.config.animation_private_offset[2]))
    else:
        logger.error("Wrong telemetry")
...
def _command_reset_z(*args, **kwargs):
    ...
    logger.info("Reset z offset to {:.2f}".format(
        copter.config.animation_private_offset[2]))
...
def _command_takeoff_z(*args, **kwargs):
    try:
        ...
        if valid([telem.x, telem.y, telem.z]):
            logger.info("Takeoff to z = {} at {}".format(
...
class Telemetry:
    ...
    def get_start_position(self):
        try:
            ...
            if not self.ros_telemetry:
                ...
                self.start_action = copter.animation.get_start_action(
                    self.ros_telemetry.z, self.fcu_status)
    ...
    def get_battery(cls, ros_telemetry):
        ...
        if batt_empty_param.success and batt_charged_param.success and batt_cells_param.success:
            ...
            battery_p = (ros_telemetry.voltage / batt_cells -
                         batt_empty) / (batt_charged - batt_empty) * 1.
    ...
    def update_telemetry_fast(self):
        ...
        try:
            self.ros_telemetry = flight.get_telemetry_locked(
                copter.config.flight_frame_id)
    ...
    def round_telemetry(self):
        ...
        for key in round_list:
            if self.__dict__[key] not in [None, 'NO_POS', 'NO_FCU']:
                self.__dict__[key] = [round(v, 2) if type(
                    v) == float else v for v in self.__dict__[key]]
    ...
    def check_failsafe_and_interruption(self):
        ...
        external_interruption = (
            mode != "OFFBOARD" and armed == True and last_task not in [None, 'land', 'stand'])
        log_msg = ''
        if emergency:
        ...
        if external_interruption:
            ...
            if state == self._last_state:
                ...
                self._interruption_counter = 0
            logger.info("Possible expernal interruption, state_counter = {}".format(
    ...
    def transmit_message(self):  # todo if connected
        try:
            copter.server_connection.send_message(
                'telemetry', kwargs={'value': self.create_msg_contents()})
        except AttributeError as e:
            logger.debug(e)
    ...
    def log_cpu_and_memory(cls):
        ...
        throttled_hex = subprocess.check_output(
            "vcgencmd get_throttled", shell=True).split('=')[1]
    ...
    def start_loop(self):
        if copter.config.telemetry_frequency > 0:
            ...
            logger.info(
                "Telemetry loop is not created because of zero or negative telemetry frequency")

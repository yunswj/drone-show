...
configspec_path = os.path.realpath(os.path.join(
    root_dir, "drone/config/spec/configspec_client.ini"))
...
def test_animation_1_2():
    a = animation.Animation(os.path.join(
        assets_dir, 'animation_1.csv'), config)
    assert a.id == 'basic'
    assert a.state == "OK"
    ...
    assert a.id == 'parad'
    assert a.state == "OK"
    assert approx(a.original_frames[271].get_pos()
                  ) == [-1.00519, 2.65699, 0.24386]
    ...
    assert approx(a.get_start_frame('fly').get_pos()) == [
        2.99481, 10.31398, 6.63]
    assert a.get_start_action() == 'takeoff'
    config.set('ANIMATION', 'ratio', [1, 1, 1])
    config.set('ANIMATION', 'common_offset', [0, 0, 0])
...
def test_animation_3():
    a = animation.Animation(os.path.join(
        assets_dir, 'animation_3.csv'), config)
    assert a.id == 'route'
    assert a.state == "OK"
...
def test_animation_4():
    a = animation.Animation(os.path.join(
        assets_dir, 'animation_4.csv'), config)

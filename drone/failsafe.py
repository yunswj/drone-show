...
logger.info('visual_pose_timeout = {} | position_delta_max = {} | watchdog_action = {}'.format(
    visual_pose_timeout, pos_delta_max, watchdog_action))
...
def emergency_land(disarm_if_timeout=True):
    ...
    while armed:
        logger.debug("Emergency land | range: {:.2f} | thrust: {:.2f}".format(
            laser_range, current_thrust))
        if current_thrust >= 0.03:
            try:
                set_attitude(thrust=current_thrust, yaw=0,
    ...
    if mode == 'OFFBOARD':
        ...
        if armed:
            ...
            if arm_time > vision_pose_delay_after_arm and watchdog_is_enabled:
                if (visual_pose_dt > visual_pose_timeout and visual_pose_timeout != 0.) or (pos_delta > pos_delta_max and pos_delta_max != 0.):
                    ...
                    if watchdog_action == 'land':
                        logger.info(
                            'Visual pose data is too old, copter is armed, landing...')
                        ...
                        logger.info(
                            'Visual pose data is too old, copter is armed, disarming...')
                        while armed:
                            try:
                                arming(False)
                            except rospy.ServiceException as e:
                        ...
                        if visual_pose_dt > visual_pose_timeout:
                            logger.info(
                                'Visual pose data is too old, copter is armed, emergency landing...')
                        if pos_delta > pos_delta_max:
                            logger.info(
                                'Position delta is {} m, copter is armed, emergency landing...'.format(pos_delta))
                        emergency_land()
            ...
            if emergency_land_called:
                emergency = True
                logger.info(
                    '/emergency_land service was called, start emergency landing...')
                emergency_land()
                logger.info('Disarmed')
                emergency = False
                emergency_land_called = False
...
rospy.Subscriber('/mavros/setpoint_position/local',
                 PoseStamped, setpoint_position_callback)

rospy.Subscriber('/mavros/setpoint_raw/local',
                 PositionTarget, setpoint_raw_callback)

...
class Animation(object):
    ...
    def load(self):
        ...
        try:
            ...
            self.set_state(
                "Bad animation delay from config 'ANIMATION' section", log_error=True)
            return
        try:
            animation_file = open(self.filepath, 'rU')
        except IOError:
            self.set_state("File {} can't be opened".format(
                self.filepath), log_error=True)
        else:
            with animation_file:
                ...
                if len(row_0) == 1:
                    ...
                    try:
                        ...
                        logger.debug("Got new frame delay: {}".format(
                            current_frame_delay))
                    except ValueError as e:
                        self.set_state(
                            "Can't parse delay row in csv file. {}".format(e), log_error=True)
                        return
                else:
                    ...
                    try:
                        ...
                        self.set_state(
                            "Can't parse frame row in csv file. {}".format(e), log_error=True)
                        return
    ...
    def set_yaw(self):
        try:
            ...
            self.set_state(
                "Can't set yaw from config 'ANIMATION' section. {}".format(e), log_error=True)
            return
    ...
    def transform(self):
        try:
            x0, y0, z0 = numpy.array(self.config.animation_common_offset) + \
                numpy.array(self.config.animation_private_offset)
        except (ValueError, KeyError):
            self.set_state(
                "Can't transform animation: bad or empty config (offset in 'ANIMATION')", log_error=True)
            return
        try:
            x_ratio, y_ratio, z_ratio = self.config.animation_ratio
        except (ValueError, KeyError):
            self.set_state(
                "Can't transform animation: bad or empty config (ratio in 'ANIMATION')", log_error=True)
            return
        self.transformed_frames = []
    ...
    def mark_stand_frames(self):
        ...
        try:
            ...
            self.set_state(
                "Can't set frame actions: bad or empty config (takeoff_level in 'ANIMATION')", log_error=True)
            return
    ...
    def apply_flags(self):
        ...
        try:
            ...
            self.set_state(
                "Can't get output flags: bad or empty config ('OUTPUT' in 'ANIMATION')", log_error=True)
            return
        try:
            takeoff_level = self.config.animation_takeoff_level
        except (ValueError, KeyError):
        ...
        if self.output_frames:
            self.output_frames_min_z = min(
                self.output_frames, key=lambda p: p.z).z
        if self.output_frames_takeoff:
            self.output_frames_takeoff_min_z = min(
                self.output_frames_takeoff, key=lambda p: p.z).z
    ...
    def mark_flight(self):
        ...
        try:
            ...
            self.set_state(
                "Can't mark flight: bad or empty config ('FLIGHT' section)", log_error=True)
            return
        ...
        self.start_time = get_duration(
            self.output_frames[:self.start_frame_index])
...
try:
    def execute_frame(frame, config, interrupter=interrupt_event):
        ...
        if frame.action == 'takeoff':
            ...
            takeoff(z=config.flight_takeoff_height, frame_id=frame_id,
                    timeout=config.flight_takeoff_time, use_leds=use_leds, interrupter=interrupter)
            return
        if frame.action == 'land':
            use_leds = use_leds & config.led_land_indication
            land(frame_id=frame_id, timeout=config.flight_land_timeout,
                 use_leds=use_leds, interrupter=interrupter)
            return
        if frame.action in ('fly', 'arm'):
            if frame.action == 'arm':
                auto_arm = True
            if frame.pose_is_valid():
                flight.navto(x=frame.x, y=frame.y, z=frame.z, yaw=frame.yaw,
                             frame_id=frame_id, auto_arm=auto_arm, interrupter=interrupter)
            else:
    ...
    def land(z=1.5, descend=False, timeout=5.0, frame_id='map', use_leds=True,
        ...
        flight.land(z=z, descend=descend, timeout_land=timeout,
                    frame_id_land=frame_id, interrupter=interrupter)
        if use_leds:

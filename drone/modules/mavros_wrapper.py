...
def calibrate(sensor):
    ...
    if check_state_topic():
        ...
        while system_status != mavlink.MAV_STATE_UNINIT:
            rospy.sleep(0.1)
        rospy.loginfo(
            "Start {} calibration. Please, don't move the drone!".format(sensor))
...
def get_amsl_altitude():
    ...
    try:
        alt_msg = rospy.wait_for_message(
            '/mavros/altitude', Altitude, timeout=0.5)
...
def get_compass_hdg():
    ...
    '''
    try:
        hdg_msg = rospy.wait_for_message(
            '/mavros/global_position/compass_hdg', Float64, timeout=0.5)
    except rospy.ROSException:
        logger.error("Can't get altitude: timeout")
        return float('nan')
    else:

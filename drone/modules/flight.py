...
def reach_point(x=0.0, y=0.0, z=0.0, yaw=float('nan'), speed=SPEED, tolerance=TOLERANCE, frame_id=FRAME_ID, auto_arm=False,
    ...
    rospy.loginfo(
        'Reaching point: | x: {:.3f} y: {:.3f} z: {:.3f} yaw: {:.3f}'.format(x, y, z, yaw))
    #print('Reaching point: | x: {:.3f} y: {:.3f} z: {:.3f} yaw: {:.3f}'.format(x, y, z, yaw))
    navigate(frame_id=frame_id, x=x, y=y, z=z,
             yaw=yaw, speed=speed, auto_arm=auto_arm)
    ...
    while (get_distance3d(x, y, z, telemetry.x, telemetry.y, telemetry.z) > tolerance) or wait:
        ...
        if timeout is not None:
            if time_passed >= timeout:
                rospy.logwarn(
                    'Reaching point timed out! | time: {:3f} seconds'.format(time_passed))
...
def reach_altitude(z=0.0, yaw=float('nan'), speed=SPEED, tolerance=TOLERANCE, frame_id=FRAME_ID,
    ...
    navigate(frame_id=frame_id, x=current_telem.x,
             y=current_telem.y, z=z, yaw=yaw, speed=speed)
    ...
    while (abs(z - telemetry.z) > tolerance) or wait:
        ...
        if timeout is not None:
            if time_passed >= timeout:
                logger.warning(
                    'Reaching attitude timed out! | time: {:3f} seconds'.format(time_passed))
...
def takeoff(height=TAKEOFF_HEIGHT, speed=TAKEOFF_SPEED, tolerance=TOLERANCE, frame_id=FRAME_ID, timeout_takeoff=TIMEOUT, interrupter=INTERRUPTER, emergency_land=False):
    ...
    climb = 0.
    time_start = time.time()
    result = navigate(z=height, speed=speed, yaw=float(
        'nan'), frame_id="body", auto_arm=True)
    ...
    while abs(climb - height) > tolerance:
        ...
        rospy.loginfo(
            "Takeoff to {:.2f} of {:.2f} meters".format(climb, height))
...
def flip(min_z=FLIP_MIN_Z, frame_id=FRAME_ID):  # TODO Flip in different directions
    ...
    start_telemetry = get_telemetry_locked(
        frame_id=frame_id)  # memorize starting position
    ...
    if start_telemetry.z < min_z - TOLERANCE:
        ...
        navto(x=start_telemetry.x, y=start_telemetry.y, z=start_telemetry.z,
              yaw=start_telemetry.yaw, frame_id=frame_id)   # finish flip

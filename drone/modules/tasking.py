...
Task = collections.namedtuple(
    "Task", ["func", "args", "kwargs", "delayable", ])
...
class TaskManager(object):
    def __init__(self):
        ...
        self._processor_thread = threading.Thread(
            target=self._task_processor, name="Task processing thread")
    ...
    def resume(self, time_to_start_next_task=0.0):
        ...
        self._running_event.set()
        logger.info(
            "Task queue resumed with timeshift {}".format(self._timeshift))
    ...
    def execute_task(self):
        ...
        logger.debug(
            "Waiting util task execution time:{}".format(task_start_time))
        ...
        if not self._wait_interrupt_event.is_set():
            ...
            try:
                task.func(
                    *task.args, interrupter=self._task_interrupt_event, **task.kwargs)

            except Exception as e:
                try:
                    logger.error(
                        "Error '{}' occurred in task {}".format(e, task))
